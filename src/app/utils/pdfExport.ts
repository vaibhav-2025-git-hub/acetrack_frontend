...
export const exportToPDF = (profile: UserProfile, plan: StudyPlan) => {
  ...
  Object.entries(profile.subjectDifficulties).forEach(([subject, difficulty]) => {
    ...
    doc.text(`${subject}: ${difficulty.charAt(0).toUpperCase() + difficulty.slice(1)} ${difficultyEmoji}`, 25, y);
    y += 6;
  });
  ...
  const sortedDates = Object.keys(plan.dailyPlans).sort();
  ...
  sortedDates.forEach((date, dateIndex) => {
    ...
    dailyPlan.sessions.forEach((session, sessionIndex) => {
      ...
      doc.setFont(undefined, 'bold');
      doc.setTextColor(0);
      doc.text(`Session ${sessionIndex + 1}: ${session.subjectName}`, 25, y + 2);
  ...
  allSessions.forEach(session => {
    if (!subjectStats[session.subjectName]) {
      subjectStats[session.subjectName] = { total: 0, completed: 0, hours: 0 };
    }
    subjectStats[session.subjectName].total++;
    subjectStats[session.subjectName].hours += session.duration;
    if (session.status === 'completed') {
      subjectStats[session.subjectName].completed++;
    }
  });
