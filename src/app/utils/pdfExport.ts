...
export const exportToPDF = (profile: UserProfile, plan: StudyPlan) => {
  ...
  const addInfo = (label: string, value: string | undefined, x: number, currentY: number) => {
    doc.setFont('helvetica', 'bold');
    doc.text(label, x, currentY);
    doc.setFont('helvetica', 'normal');
    doc.text(value || 'N/A', x + 35, currentY);
    return currentY + 5;
  };

  let leftY = y;
  leftY = addInfo('Name:', profile.name || 'Student', leftColX, leftY);
  leftY = addInfo('Class:', `${profile.class || 'N/A'}`, leftColX, leftY);
  leftY = addInfo('Board:', (profile.board || '').toUpperCase(), leftColX, leftY);
  leftY = addInfo('Stream:', profile.stream || 'N/A', leftColX, leftY);
  leftY = addInfo('Study/Day:', `${profile.studyHoursPerDay} hrs`, leftColX, leftY);
  leftY = addInfo('Pace:', (profile.learningSpeed || 'medium').charAt(0).toUpperCase() + (profile.learningSpeed || 'medium').slice(1), leftColX, leftY);
  leftY = addInfo('Duration:', `${profile.totalDays || 0} days`, leftColX, leftY);
  ...
  sortedDates.forEach((date, dateIndex) => {
    ...
    dailyPlan.sessions.forEach((session) => {
      ...
      const statusColor: [number, number, number] = session.status === 'completed' ? [34, 197, 94] :
